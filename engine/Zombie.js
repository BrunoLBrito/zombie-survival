class Zombie {
  ...
  eyes() {
    for (let x = -1; x <= 1; x += 2) {
      ...
      ctx.arc(
        this.x + (Math.cos(this.angle + (x * 40 * Math.PI) / 180) * this.radiusStart) / 2,
        this.y + (Math.sin(this.angle + (x * 40 * Math.PI) / 180) * this.radiusStart) / 2,
        this.radiusStart / 4,
        0,
        2 * Math.PI
      )
  ...
  hp() {
    ctx.save()
    ctx.fillStyle = 'red'
    ctx.fillRect(this.x - this.sizeBarHP / 2, this.y - this.radiusStart - 10, this.sizeBarHP, 4)
    ctx.restore()
